...
데이터는 1건부터 시작해서 여러건 일 수 있습니다.
{
    ...
    "gender" : "성별 입니다. 성별에는 남자/여자만 존재합니다. 값이 판단이 되지 않을 경우 남자를 기본값으로 해주세요. 문자열 타입입니다.",
    "ageRange" : "연령대 입니다. 숫자 타입입니다. 10, 20, 30, 40, 50, 60, 70, 80, 90의 값이 올 수 있습니다.",
...
//"""
    ...
    public func scanImageData(_ data: Data, model: ChatGPTModel = .gpt_hyphen_4o, temperature: Double = 1.0) async throws -> FingerReceipt {
        do {
            ...
            printDebug(response)
            let receipt = try dateFormatterJSONDecoder.decode(FingerReceipt.self, from: data)
            return receipt
        } catch {
